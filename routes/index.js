...
router.post('/download', async function (req, res) {
  ...
  try {
    ...
    const videoStream = ytdl(url, { quality: itag , requestOptions: { proxy: `http://localhost:${process.env.PORT}` } });
    const audioStream = ytdl(url, { quality: 'highestaudio' , requestOptions: { proxy: `http://localhost:${process.env.PORT}` }});
